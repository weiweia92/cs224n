...
def naiveSoftmaxLossAndGradient(
	...
	value = np.dot(outsideVectors, centerWordVec) #m×1=(m×len_centerword).(len_centerword,1)
	y_hat = softmax(value) #m×1
	loss = -np.log(y_hat[outsideWordIdx])
	
	d_value = y_hat
	d_value[outsideWordIdx] -= 1 #y_hat - y, matrix shape (m, 1)
	gradCenterVec = outsideVectors.T.dot(d_value) # shape d×1
	gradOutsideVecs = d_value[:, np.newaxis].dot(np.array([centerWordVec])) # (m,1).(1,d)->(m,d)
...
def negSamplingLossAndGradient(
	...
	for i, neg_index in enumerate(negSampleWordIndices):
		gradOutsideVecs[neg_index] += (1 - p_negative[i]) * centerWordVec # remember negative can appear multiple time
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
	...
	for outsideWordIdx in outsideWordIndices:
		...
		gradOutsideVecs += one_gradOutside
    ### END YOUR CODE
